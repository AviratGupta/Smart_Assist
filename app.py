...
@app.route("/spell_checker", methods=['GET'])
def checker():
...
def check():
    if request.method == 'POST':
        ...
        print(result)
    return render_template('spell_checker.html',correct_text = result,actual = text)
...
def translat():
    if request.method == 'POST':
        text = request.form.get("input_text_translate")
        inputl = request.form.get('Ilanguage')
        outputl = request.form.get('Olanguage')
        result = translator(text,inputl,outputl)
        print(result)
    return render_template('translator.html',translated_text = result,actual = text)
